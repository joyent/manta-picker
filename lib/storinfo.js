 ...
 * instances hitting shard 1 to get storage utilization data, the manta-storinfo
...
function sortAndStoreDcs(dcObj, opDcObj) {
    ...
    if (dcObj.length === 0) {
        this.log.warn('Storinfo.sortAndStoreDcs: could not find any minnow ' +
            'instances');
    }
    ...
    this.log.trace('Storinfo.sortAndStoreDcs: done');
}
 ...
 * This function requires that "this" be bound to an instance of Storinfo.
 */
function handleStorageResults(err, storageZoneResults) {
    ...
    if (err) {
         ...
         * a retry.
         */
        this.log.error(err, 'Storinfo.handleStorageResults: unexpected error ' +
            '(will retry)');
        return;
    }
 ...
 * requires that "this" be bound to an instance of Storinfo. This period is
 * determined by the value of storageInterval established when the Storinfo
 * instance is created.
 */
function pollStorage() {
    ...
    this.log.trace('Storinfo.pollStorage: entered');
    clearTimeout(this._storageTimer);
...
/**
 * Creates an instance of storinfo, and an underlying moray client.
 *
...
function Storinfo(opts) {
    ...
    this.log = opts.log.child({component: 'storinfo'}, true);
    ...
    /*
     * `Standalone` mode is used only when an instance of the Storinfo is needed
...
util.inherits(Storinfo, EventEmitter);


Storinfo.prototype.close = function close() {
    clearTimeout(this._storageTimer);
    if (this.client)
        this.client.close();
};
...
Storinfo.prototype.toString = function toString() {
    var str = '[object Storinfo <';
...
Storinfo.prototype.flush = function flush() {
